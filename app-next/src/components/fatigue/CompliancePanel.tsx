...
export default function CompliancePanel({
  ...
  return (
    <div className="space-y-4">
      <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${isTwoUp ? "bg-purple-100 text-purple-700" : "bg-slate-100 text-slate-600"}`}>
      ...
      <div className={`rounded-lg p-2.5 ${ACTIVITY_THEME.rest.statsCard} border border-emerald-100`}>
        <p className="text-[10px] uppercase tracking-wider font-semibold text-emerald-600 mb-1.5">Non-Work Time rules</p>
      ...
      {checks.length === 0 && (
        <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-lg p-3">
          ...
          <span className="text-sm font-medium text-emerald-700">All compliant — no issues detected</span>
        </div>
      )}
      <AnimatePresence>
        {violations.length > 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-1.5">
            <p className="text-[10px] uppercase tracking-wider text-red-500 font-bold">Violations ({violations.length})</p>
            {violations.map((v, i) => {
                const Icon = ICON_MAP[v.iconKey];
                return (
              <div key={i} className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-lg p-2.5">
                {Icon && <Icon className="w-4 h-4 text-red-500 mt-0.5 shrink-0" />}
                <div>
      ...
      <AnimatePresence>
        {warnings.length > 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-1.5">
            <p className="text-[10px] uppercase tracking-wider text-amber-500 font-bold">Warnings ({warnings.length})</p>
            {warnings.map((w, i) => {
              ...
              <div key={i} className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg p-2.5">
                {Icon && <Icon className="w-4 h-4 text-amber-500 mt-0.5 shrink-0" />}
                <div>
